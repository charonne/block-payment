...
class Bitcoin
{
     ...
     * Validate address
     */
    static public function validate($addr, $version = null)
	{
	 ...
	 * Get dollars value
	*/
	static public function getDollarRate()
	{
		$json = file_get_contents('https://api.coindesk.com/v1/bpi/currentprice/CNY.json');
        $obj = json_decode($json);
        if (isset($obj->bpi->USD->rate)) {
            $dollarRate = $obj->bpi->USD->rate;
        } else {
            $this->output->writeln("Get dollar rates failed");
            $dollarRate = 0;
        }
		return $dollarRate;
	}
	 ...
	 * Get dollars value
	*/
	public function getDollarPrice($amount)
	{
		$dollarRate = $this->getDollarRate();
        $dollarPrice = $amount * $dollarRate;
